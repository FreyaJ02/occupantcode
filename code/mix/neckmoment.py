...
def define_hybrid_model(input_length=100, scalar_length=3, output_length=100):
    ...
    model = Model(inputs=[input_lstm, input_dense], outputs=neck_moment_output)
    return model
...
def load_filtered_npy_data(data_dir):
    ...
    assert airbag.shape[0] == gender.shape[0] == seatbelt.shape[0] == crash_pulse.shape[0] == neck_moment.shape[0], \
...
def train_model_two_stage(model,
                          X_train_lstm, X_train_dense, y_train_neck_moment,
                          X_val_lstm, X_val_dense, y_val_neck_moment,
                          save_dir):
    ...
    history_stage1 = model.fit(
        ...
        batch_size=16,
        validation_data=([X_val_lstm, X_val_dense], y_val_neck_moment),
        callbacks=[
...
def main():
    ...
    save_dir = '/public/home/acnebvshuq/js/occupant/norm/result/mix/neckmoment'
    os.makedirs(save_dir, exist_ok=True)
    ...
    assert scalar_data.shape[0] == crash_pulse.shape[0] == neck_moment.shape[0], 'Sample count mismatch.'
    ...
    y_neck_moment_normalized = scaler_neck_moment.fit_transform(neck_moment.reshape(-1, 1)).reshape(neck_moment.shape)
    ...
    y_test_neck_moment_normalized = y_neck_moment_normalized[test_indices]
    y_test_neck_moment_original = neck_moment[test_indices]
    ...
    model.compile(loss=ISO18571Loss_DTW(), optimizer=tf.keras.optimizers.Adam(learning_rate=1e-4))
    result = model.evaluate([X_test_crash, X_test_scalar], y_test_neck_moment_normalized, verbose=1)
    print(f'Test Loss: {result}')

    y_pred_neck_moment_normalized = model.predict([X_test_crash, X_test_scalar], verbose=1)

    y_pred_neck_original = scaler_neck_moment.inverse_transform(
        y_pred_neck_moment_normalized.reshape(-1, 1)
    ).reshape(y_pred_neck_moment_normalized.shape)

    y_test_neck_moment_true = y_test_neck_moment_original
    y_test_neck_moment_pred = y_pred_neck_original
    ...
    with open(result_file_path, 'w', encoding='utf-8') as f:
        ...
        for i in range(len(y_test_neck_moment_true)):
            print_and_write(f'\nSample {i + 1} ISO score:')
            score, corridor, phase, magnitude, slope = calculate_iso_scores_for_sample(
                y_test_neck_moment_true,
                y_test_neck_moment_pred,
                time_axis,
                i,
                'NeckMoment',
                f,
            )
            neck_moment_iso_scores.append(score)
            neck_moment_corridor_scores.append(corridor)
            neck_moment_phase_scores.append(phase)
            neck_moment_magnitude_scores.append(magnitude)
            neck_moment_slope_scores.append(slope)

        def valid(scores):
            return [s for s in scores if s is not None]

        valid_scores = valid(neck_moment_iso_scores)
        if valid_scores:
            print_and_write(
                'NeckMoment average ISO score: '
                f"{np.mean(valid_scores):.2f} "
                f"(Corridor: {np.mean(valid(neck_moment_corridor_scores)):.2f}, "
                f"Phase: {np.mean(valid(neck_moment_phase_scores)):.2f}, "
                f"Magnitude: {np.mean(valid(neck_moment_magnitude_scores)):.2f}, "
                f"Slope: {np.mean(valid(neck_moment_slope_scores)):.2f})"
            )
        else:
            print_and_write('NeckMoment - no valid ISO score')
    ...
    for i in range(min(20, len(y_test_neck_moment_true))):
        ...
        if i < len(neck_moment_iso_scores) and neck_moment_iso_scores[i] is not None:
            ax.text(
                0.02,
                0.98,
                f'ISO Score: {neck_moment_iso_scores[i]:.2f}',
