...
def define_hybrid_model(input_length=100, scalar_length=3, output_length=100):
    ...
    注意：
    本模型输出的是归一化后的ChestDisplacement。
    后续预测结果需要通过scaler_chest进行反归一化。
    """
    # 输入层
...
def train_model_two_stage(model,
                          X_train_lstm, X_train_dense, y_train_chest,
                          X_val_lstm, X_val_dense, y_val_chest,
                          save_dir):
    """
    两阶段训练：
    ...
    注意：
        两个阶段均使用归一化后的y_train_chest和y_val_chest。
    """
    ...
    history_stage1 = model.fit(
        ...
        batch_size=16,
        validation_data=([X_val_lstm, X_val_dense], y_val_chest),
        callbacks=[
            stage1_lr_scheduler,
            stage1_checkpoint,
            stage1_timing
...
def main():
    ...
    save_dir = '/public/home/acnebvshuq/js/occupant/norm/result/mix/chest'
    os.makedirs(save_dir, exist_ok=True)
    ...
    y_test_chest_normalized = y_chest_normalized[test_indices]
    ...
    # 预测并反归一化
    # =========================================================
    y_pred_chest_normalized = model.predict(
        [X_test_crash, X_test_scalar],
        verbose=1
    )

    y_pred_chest_flat = y_pred_chest_normalized.reshape(-1, 1)
    y_test_chest_norm_flat = y_test_chest_normalized.reshape(-1, 1)

    y_pred_chest_original = scaler_chest.inverse_transform(
        y_pred_chest_flat
    ).reshape(y_pred_chest_normalized.shape)

    y_test_chest_from_norm = scaler_chest.inverse_transform(
        y_test_chest_norm_flat
    ).reshape(y_test_chest_normalized.shape)

    y_test_chest_true = y_test_chest_original
    y_test_chest_pred = y_pred_chest_original

    # =========================================================
    # 计算MSE和MAE
    # =========================================================
    chest_mse = mean_squared_error(
        y_test_chest_true.flatten(),
        y_test_chest_pred.flatten()
    )

    chest_mae = np.mean(
        np.abs(y_test_chest_true - y_test_chest_pred)
    )
    ...
    with open(result_file_path, 'w', encoding='utf-8') as f:
        ...
        print_and_write("两个阶段均使用归一化后的ChestDisplacement进行训练")
        ...
        print_and_write(f"ChestDisplacement MSE: {chest_mse}")
        print_and_write(f"ChestDisplacement MAE: {chest_mae}")
        ...
        num_samples_to_calc = len(y_test_chest_true)
        ...
        if valid_chest_scores:
            ...
            print_and_write(
                f"ChestDisplacement - 平均ISO评分: {avg_chest_iso:.2f} "
                f"(Corridor: {avg_chest_corridor:.2f}, "
                f"Phase: {avg_chest_phase:.2f}, "
                f"Magnitude: {avg_chest_magnitude:.2f}, "
                f"Slope: {avg_chest_slope:.2f})"
            )
        else:
    ...
    for i in range(num_samples_to_plot):
        ...
        if i < len(chest_iso_scores) and chest_iso_scores[i] is not None:
            ax.text(
                0.02,
                0.98,
                f'ISO Score: {chest_iso_scores[i]:.2f}',
