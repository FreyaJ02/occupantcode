...
class ISO18571Loss_DTW(tf.keras.losses.Loss):
    def __init__(
        ...
        # ===== 平滑评分相关参数（优化版）=====
        smooth_beta=30.0,        # 增加平滑强度：从25→30，使梯度更稳定，减少极端低分样本

        corridor_alpha=3.5,      # 增强走廊基础惩罚：从2.0→3.5，提升走廊符合度（原低9.4%）
        corridor_lambda=12.0,    # 增强外走廊惩罚：从8.0→12.0，严格控制超出范围的情况

        magnitude_alpha=1.5,     # 适度增强幅值基础惩罚：从1.0→1.5，改善幅值精度（原低3.8%）
        magnitude_lambda=8.0,    # 增强超阈值惩罚：从6.0→8.0，抑制大幅值误差

        slope_alpha=1.2,         # 轻微增强斜率基础惩罚：从1.0→1.2，保持斜率优势
        slope_lambda=7.0,        # 适度增强斜率超阈值惩罚：从6.0→7.0
        slope_power=1.2,         # 增加斜率幂指数：从1.0→1.2，对大误差更敏感

        phase_alpha=3.5,         # 显著增强相位惩罚：从2.0→3.5，解决相位对齐问题（原低10.3%）
