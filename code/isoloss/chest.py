...
from isoloss_new2 import ISO18571Loss_DTW
#from isoloss_soft_new import ISO18571Loss_DTW
# 设置随机种子以确保可复现性
np.random.seed(42)
...
def define_hybrid_model(input_length=100, scalar_length=3, output_length=100):
    ...
    # 单个输出分支 - 仅胸部位移
    chest_output = Dense(output_length, name='chest_output')(shared)  # ChestDisplacement

    # 创建单输出模型
    model = Model(inputs=[input_lstm, input_dense],
                  outputs=chest_output)
...
def load_filtered_npy_data(data_dir):
    ...
    # 加载时序输出数据 - 仅胸部位移
    chest_displacement = np.load(os.path.join(data_dir, 'ChestDisplacement.npy'))

    # 检查数据维度是否一致
    assert airbag.shape[0] == gender.shape[0] == seatbelt.shape[0] == \
           crash_pulse.shape[0] == chest_displacement.shape[0], "所有数据文件的第一维度必须一致"

    # 组装标量数据
    scalar_data = np.column_stack([airbag, gender, seatbelt])

    return scalar_data, crash_pulse, chest_displacement
...
def train_model(model, X_train_lstm, X_train_dense, y_train_chest, X_val_lstm, X_val_dense,
                y_val_chest, save_dir):
    """
    训练模型
    """
...
def main():
    ...
    save_dir = '/public/home/acnebvshuq/js/occupant/norm/result/isoloss/chest_new2'
    os.makedirs(save_dir, exist_ok=True)

    # 加载数据（函数名虽然叫load_filtered_npy_data，但实际上加载的是未过滤的数据）
    print("加载数据...")
    scalar_data, crash_pulse, chest_displacement = load_filtered_npy_data(data_dir)

    # 检查数据形状
    print(f"标量数据形状: {scalar_data.shape}")
    print(f"CrashPulse形状: {crash_pulse.shape}")
    print(f"ChestDisplacement形状: {chest_displacement.shape}")

    # 检查数据形状是否匹配
    total_samples = scalar_data.shape[0]
    assert scalar_data.shape[0] == crash_pulse.shape[0] == chest_displacement.shape[0], \
           "所有数据的样本数量必须一致"

    # 仅对时序数据进行归一化，标量数据保持原样
    scaler_crash = MinMaxScaler()
    scaler_chest = MinMaxScaler(feature_range=(-1, 0))  # 胸部位移归一化到(-1, 0)
    ...
    # 归一化胸部位移输出数据
    # 重塑ChestDisplacement为(total_samples*100, 1)用于归一化
    chest_displacement_reshaped = chest_displacement.reshape(-1, 1)
    y_chest_normalized = scaler_chest.fit_transform(chest_displacement_reshaped).reshape(chest_displacement.shape)
    ...
    y_train_chest = y_chest_normalized[train_indices]

    # 验证集
    X_val_crash = X_crash[val_indices]
    X_val_scalar = X_scalar[val_indices]
    y_val_chest = y_chest_normalized[val_indices]

    # 测试集
    X_test_crash = X_crash[test_indices]
    X_test_scalar = X_scalar[test_indices]
    y_test_chest = y_chest_normalized[test_indices]
    y_test_chest_original = chest_displacement[test_indices]  # 保存原始值用于评估
    ...
    # 保存胸部位移的scaler，用于后续反归一化
    import joblib
    scaler_path = os.path.join(save_dir, 'chest_scaler.pkl')
    joblib.dump(scaler_chest, scaler_path)
    print(f"胸部位移归一化器已保存至: {scaler_path}")
    ...
    if os.path.exists(best_model_path):
        ...
        model = load_model(best_model_path, custom_objects={'ISO18571Loss_DTW': ISO18571Loss_DTW})
        # 重新编译模型
        model.compile(
            loss=ISO18571Loss_DTW(),
            optimizer=tf.keras.optimizers.Adam(learning_rate=1e-3)
    ...
    # 预测
    y_pred_chest_normalized = model.predict([X_test_crash, X_test_scalar])

    # 反归一化预测结果和真实值
    # 重塑为(-1, 1)以适配scaler
    pred_shape = y_pred_chest_normalized.shape
    y_pred_reshaped = y_pred_chest_normalized.reshape(-1, 1)
    y_test_reshaped = y_test_chest.reshape(-1, 1)
    
    # 反归一化
    y_pred_chest = scaler_chest.inverse_transform(y_pred_reshaped).reshape(pred_shape)
    y_test_chest_true = scaler_chest.inverse_transform(y_test_reshaped).reshape(pred_shape)
    y_test_chest_pred = y_pred_chest  # 统一变量名，用于后续ISO评分计算
    
    # 也可以使用之前保存的原始值
    # y_test_chest_true = y_test_chest_original

    # 计算评估指标
    chest_mse = mean_squared_error(y_test_chest_true.flatten(), y_pred_chest.flatten())
    chest_mae = np.mean(np.abs(y_test_chest_true - y_pred_chest))
    ...
    with open(result_file_path, 'w', encoding='utf-8') as f:
        ...
        # 计算每个样本的ISO评分
        num_samples_to_calc = len(y_test_chest_true)  # 计算所有测试样本

        for i in range(num_samples_to_calc):
            print_and_write(f"\nSample {i + 1} ISO评分:")

            # 计算ChestDisplacement的ISO评分
            chest_score, chest_corridor, chest_phase, chest_magnitude, chest_slope = calculate_iso_scores_for_sample(
                y_test_chest_true, y_test_chest_pred, time_axis, i, "ChestDisplacement", f)
            chest_iso_scores.append(chest_score)
            chest_corridor_scores.append(chest_corridor)
            chest_phase_scores.append(chest_phase)
            chest_magnitude_scores.append(chest_magnitude)
            chest_slope_scores.append(chest_slope)
        ...
        # 计算ChestDisplacement的平均评分
        valid_chest_scores = filter_valid_scores(chest_iso_scores)
        if valid_chest_scores:
            avg_chest_iso = np.mean(valid_chest_scores)
            avg_chest_corridor = np.mean(filter_valid_scores(chest_corridor_scores))
            avg_chest_phase = np.mean(filter_valid_scores(chest_phase_scores))
            avg_chest_magnitude = np.mean(filter_valid_scores(chest_magnitude_scores))
            avg_chest_slope = np.mean(filter_valid_scores(chest_slope_scores))
            print_and_write(
                f"ChestDisplacement - 平均ISO评分: {avg_chest_iso:.2f} (Corridor: {avg_chest_corridor:.2f}, Phase: {avg_chest_phase:.2f}, Magnitude: {avg_chest_magnitude:.2f}, Slope: {avg_chest_slope:.2f})")
        else:
            print_and_write("ChestDisplacement - 没有有效的ISO评分")

        # 将五个ISO指标统一可视化为一张分布直方图
        detailed_metrics = {
            'corridor': chest_corridor_scores,
            'phase': chest_phase_scores,
            'magnitude': chest_magnitude_scores,
            'slope': chest_slope_scores,
            'overall': chest_iso_scores
        }
    ...
    num_samples_to_plot = min(20, len(y_test_chest_true))
    for i in range(num_samples_to_plot):
        fig, ax = plt.subplots(1, 1, figsize=(12, 6))

        # ChestDisplacement
        ax.plot(y_test_chest_true[i], 'g-', label='True', linewidth=1, alpha=0.8)
        ax.plot(y_pred_chest[i], 'r--', label='Predict', linewidth=1, alpha=0.8)

        # 添加ISO评分标注
        if i < len(chest_iso_scores) and chest_iso_scores[i] is not None:
            ax.text(0.02, 0.98, f'ISO Score: {chest_iso_scores[i]:.2f}',
                    transform=ax.transAxes, fontsize=10,
                    verticalalignment='top', horizontalalignment='left',
                    bbox=dict(boxstyle='round', facecolor='white', alpha=0.8))

        ax.set_title(f'ChestDisplacement - Sample {i + 1}')
        ax.set_xlabel('Time Steps')
        ax.set_ylabel('Chest Displacement')
        ax.legend()
