...
def define_hybrid_model(input_length=100, scalar_length=3, output_length=100):
    ...
    仅预测头部加速度曲线（HeadAcceleration）
    注意：输出层使用线性激活，模型输出为归一化后的值(0,1)，需要反归一化才能得到原始值
    """
    # 输入层
    ...
    # 单个输出分支 - 仅头部加速度
    # 使用线性激活（默认），确保输出保持原始值范围，不进行归一化
    head_output = Dense(output_length, name='head_output')(shared)  # HeadAcceleration
...
def train_model(model, X_train_lstm, X_train_dense, y_train_head, X_val_lstm, X_val_dense,
                y_val_head, save_dir):
    """
    训练模型
    注意：y_train_head 和 y_val_head 均为归一化后的值(0,1)
    """
...
def calculate_iso_scores_for_sample(y_true, y_pred, time_axis, sample_idx, output_name, f):
    """
    计算单个样本的ISO评分
    注意：y_true 和 y_pred 均为原始值，未进行归一化处理
    """
...
def main():
    ...
    save_dir = '/public/home/acnebvshuq/js/occupant/norm/result/isoloss/headacc'
    os.makedirs(save_dir, exist_ok=True)
    ...
    y_test_head_normalized = y_head_normalized[test_indices]  # 归一化值，用于模型评估
    y_test_head_original = head_acc[test_indices]  # 原始值，用于ISO评分计算
    ...
    # 预测 - 输出为归一化值(0,1)，需要反归一化
    y_pred_head_normalized = model.predict([X_test_crash, X_test_scalar])

    # 反归一化：将预测值和真实值从(0,1)恢复到原始值范围
    # 重塑为(-1, 1)以进行反归一化
    y_pred_head_flat = y_pred_head_normalized.reshape(-1, 1)
    y_test_head_norm_flat = y_test_head_normalized.reshape(-1, 1)
    
    # 反归一化
    y_pred_head_original = scaler_head.inverse_transform(y_pred_head_flat).reshape(y_pred_head_normalized.shape)
    y_test_head_from_norm = scaler_head.inverse_transform(y_test_head_norm_flat).reshape(y_test_head_normalized.shape)
    
    # 使用反归一化后的值进行评估和可视化
    y_test_head_true = y_test_head_original  # 原始真实值
    y_test_head_pred = y_pred_head_original  # 反归一化后的预测值
    ...
    with open(result_file_path, 'w', encoding='utf-8') as f:
        ...
        # 计算每个样本的ISO评分
        num_samples_to_calc = len(y_test_head_true)  # 计算所有测试样本

        for i in range(num_samples_to_calc):
            print_and_write(f"\nSample {i + 1} ISO评分:")

            # 计算HeadAcceleration的ISO评分 - 使用原始值
        ...
        if valid_head_scores:
            ...
            print_and_write(
                f"HeadAcceleration - 平均ISO评分: {avg_head_iso:.2f} (Corridor: {avg_head_corridor:.2f}, Phase: {avg_head_phase:.2f}, Magnitude: {avg_head_magnitude:.2f}, Slope: {avg_head_slope:.2f})")
        else:
    ...
    for i in range(num_samples_to_plot):
        ...
        # HeadAcceleration - 使用原始值绘制
        ax.plot(y_test_head_true[i], 'g-', label='True', linewidth=1, alpha=0.8)
        ax.plot(y_test_head_pred[i], 'r--', label='Predict', linewidth=1, alpha=0.8)

        # 添加ISO评分标注
        if i < len(head_iso_scores) and head_iso_scores[i] is not None:
            ax.text(0.02, 0.98, f'ISO Score: {head_iso_scores[i]:.2f}',
