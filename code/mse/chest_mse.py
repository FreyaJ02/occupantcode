...
def define_hybrid_model(input_length=100, scalar_length=3, output_length=100):
    ...
    注意：输出层使用线性激活，模型输出为归一化后的值(-1,0)，需要反归一化才能得到原始值
    """
    # 输入层
...
def train_model(model, X_train_lstm, X_train_dense, y_train_chest, X_val_lstm, X_val_dense,
    ...
    """
    训练模型
    注意：y_train_chest 和 y_val_chest 均为归一化后的值(-1,0)
    """
...
def main():
    ...
    save_dir = '/public/home/acnebvshuq/js/occupant/norm/result/mse/chest'
    os.makedirs(save_dir, exist_ok=True)
    ...
    y_test_chest_normalized = y_chest_normalized[test_indices]  # 归一化值，用于模型评估
    y_test_chest_original = chest_displacement[test_indices]  # 原始值，用于ISO评分计算
    ...
    # 预测 - 输出为归一化值(-1,0)，需要反归一化
    y_pred_chest_normalized = model.predict([X_test_crash, X_test_scalar])

    # 反归一化：将预测值和真实值从(-1,0)恢复到原始值范围
    # 重塑为(-1, 1)以进行反归一化
    y_pred_chest_flat = y_pred_chest_normalized.reshape(-1, 1)
    y_test_chest_norm_flat = y_test_chest_normalized.reshape(-1, 1)
    
    # 反归一化
    y_pred_chest_original = scaler_chest.inverse_transform(y_pred_chest_flat).reshape(y_pred_chest_normalized.shape)
    y_test_chest_from_norm = scaler_chest.inverse_transform(y_test_chest_norm_flat).reshape(y_test_chest_normalized.shape)
    
    # 使用反归一化后的值进行评估和可视化
    y_test_chest_true = y_test_chest_original  # 原始真实值
    y_test_chest_pred = y_pred_chest_original  # 反归一化后的预测值
    ...
    for i in range(num_samples_to_plot):
        ...
        ax.plot(y_test_chest_pred[i], 'r--', label='Predict', linewidth=1, alpha=0.8)
