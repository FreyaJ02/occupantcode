...
def define_hybrid_model(input_length=100, scalar_length=3, output_length=100):
    ...
    # 单个输出分支 - 仅颈部力
    neck_force_output = Dense(output_length, name='neck_force_output')(shared)  # NeckForce

    # 创建单输出模型
    model = Model(inputs=[input_lstm, input_dense],
                  outputs=neck_force_output)
...
def load_filtered_npy_data(data_dir):
    ...
    assert airbag.shape[0] == gender.shape[0] == seatbelt.shape[0] == \
           crash_pulse.shape[0] == neck_force.shape[0], "所有数据文件的第一维度必须一致"
...
def train_model(model, X_train_lstm, X_train_dense, y_train_neck_force, X_val_lstm, X_val_dense,
                y_val_neck_force, save_dir):
    """
    训练模型
    注意：y_train_neck_force 和 y_val_neck_force 均为归一化后的值(-1,1)
    """
...
def plot_training_history(history, save_dir):
    ...
    # 定义所有输出名称
    output_names = ['neck_force_output']
    colors = ['b']
...
def main():
    ...
    save_dir = '/public/home/acnebvshuq/js/occupant/norm/result/mse/neckforce'
    os.makedirs(save_dir, exist_ok=True)
    ...
    assert scalar_data.shape[0] == crash_pulse.shape[0] == neck_force.shape[0], \
           "所有数据的样本数量必须一致"
    ...
    neck_force_reshaped = neck_force.reshape(-1, 1)
    y_neck_force_normalized = scaler_neck_force.fit_transform(neck_force_reshaped).reshape(neck_force.shape)
    ...
    y_test_neck_force_normalized = y_neck_force_normalized[test_indices]  # 归一化值，用于模型评估
    y_test_neck_force_original = neck_force[test_indices]  # 原始值，用于ISO评分计算
    ...
    # 保存颈部力的scaler，用于后续反归一化
    import joblib
    scaler_path = os.path.join(save_dir, 'neck_force_scaler.pkl')
    joblib.dump(scaler_neck_force, scaler_path)
    print(f"颈部力归一化器已保存至: {scaler_path}")
    ...
    result = model.evaluate([X_test_crash, X_test_scalar],
                            y_test_neck_force_normalized)
    print(f"Test Losses: {result}")

    # 预测 - 输出为归一化值(-1,1)，需要反归一化
    y_pred_neck_force_normalized = model.predict([X_test_crash, X_test_scalar])

    # 反归一化：将预测值和真实值从(-1,1)恢复到原始值范围
    # 重塑为(-1, 1)以进行反归一化
    y_pred_neck_flat = y_pred_neck_force_normalized.reshape(-1, 1)
    y_test_neck_norm_flat = y_test_neck_force_normalized.reshape(-1, 1)
    
    # 反归一化
    y_pred_neck_original = scaler_neck_force.inverse_transform(y_pred_neck_flat).reshape(y_pred_neck_force_normalized.shape)
    y_test_neck_from_norm = scaler_neck_force.inverse_transform(y_test_neck_norm_flat).reshape(y_test_neck_force_normalized.shape)
    
    # 使用反归一化后的值进行评估和可视化
    y_test_neck_force_true = y_test_neck_force_original  # 原始真实值
    y_test_neck_force_pred = y_pred_neck_original  # 反归一化后的预测值
    ...
    with open(result_file_path, 'w', encoding='utf-8') as f:
        ...
        # 计算每个样本的ISO评分
        num_samples_to_calc = len(y_test_neck_force_true)  # 计算所有测试样本

        for i in range(num_samples_to_calc):
            print_and_write(f"\nSample {i + 1} ISO评分:")

            # 计算NeckForce的ISO评分
            neck_force_score, neck_force_corridor, neck_force_phase, neck_force_magnitude, neck_force_slope = calculate_iso_scores_for_sample(
                y_test_neck_force_true, y_test_neck_force_pred, time_axis, i, "NeckForce", f)
            neck_force_iso_scores.append(neck_force_score)
            neck_force_corridor_scores.append(neck_force_corridor)
            neck_force_phase_scores.append(neck_force_phase)
            neck_force_magnitude_scores.append(neck_force_magnitude)
            neck_force_slope_scores.append(neck_force_slope)
        ...
        # 计算NeckForce的平均评分
        valid_neck_force_scores = filter_valid_scores(neck_force_iso_scores)
        if valid_neck_force_scores:
            avg_neck_force_iso = np.mean(valid_neck_force_scores)
            avg_neck_force_corridor = np.mean(filter_valid_scores(neck_force_corridor_scores))
            avg_neck_force_phase = np.mean(filter_valid_scores(neck_force_phase_scores))
            avg_neck_force_magnitude = np.mean(filter_valid_scores(neck_force_magnitude_scores))
            avg_neck_force_slope = np.mean(filter_valid_scores(neck_force_slope_scores))
            print_and_write(
                f"NeckForce - 平均ISO评分: {avg_neck_force_iso:.2f} (Corridor: {avg_neck_force_corridor:.2f}, Phase: {avg_neck_force_phase:.2f}, Magnitude: {avg_neck_force_magnitude:.2f}, Slope: {avg_neck_force_slope:.2f})")
        else:
    ...
    num_samples_to_plot = min(20, len(y_test_neck_force_true))
    for i in range(num_samples_to_plot):
        ...
        if i < len(neck_force_iso_scores) and neck_force_iso_scores[i] is not None:
            ax.text(0.02, 0.98, f'ISO Score: {neck_force_iso_scores[i]:.2f}',
